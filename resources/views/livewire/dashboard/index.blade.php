<div class="py-12">
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden sm:rounded-lg px-6 py-4 shadow sm:rounded-tl-md sm:rounded-tr-md">
                    <div class="pt-4 px-5 flex">
                        ...
                        <div class="w-full md:w-1/2 flex justify-end">
                            <span class="inline-flex items-center justify-center rounded-full bg-gray-100 px-2.5 py-0.5 text-gray-700 mr-2">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="-ms-1 me-1.5 h-5 w-5">
        ...
        <div class="w-full py-12">
            <div class="bg-white dark:bg-gray-800 overflow-hidden sm:rounded-lg px-6 py-4 shadow sm:rounded-tl-md sm:rounded-tr-md">
                <div class="pt-7 pb-4 px-5 mb-2 flex">
                    ...
                    <div class="w-full md:w-1/2 flex justify-end">
                        <a wire:navigate href="{{ route('devices') }}" class="group relative inline-flex items-center overflow-hidden rounded border border-current px-6 py-2 text-gray-600 focus:outline-none focus:ring active:text-gray-500">
                            <span class="absolute -start-full transition-all group-hover:start-3">
                                <svg class="size-5 rtl:rotate-180" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-4 h-4">
                    ...
                    <div class="overflow-x-auto rounded-t-lg rounded-b-lg">
                        <table class="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
                            ...
                            <tbody class="divide-y divide-gray-200">
                                ...
                                <tr>
                                    <td class="whitespace-nowrap px-4 py-2 font-medium text-gray-700">{{ $device['hostname'] }}</td>
                                    ...
                                    <td class="whitespace-nowrap px-4 py-2 font-medium text-gray-700"><code>{{ $device['mgmt_ipaddr'] }}</code></td>
                        ...
                        <div class="flow-root">
                            <dl class="-my-3 divide-y divide-gray-100 text-sm">
                                <div class="grid grid-cols-1 gap-1 py-3 sm:grid-cols-3 sm:gap-4">
                                    ...
                                    <dd class="text-gray-700 sm:col-span-2 overflow-auto"><code>{{ $buildInformation['prometheus']['version'] ?? 'unknown' }}</code></dd>
                                </div>
                                <div class="grid grid-cols-1 gap-1 py-3 sm:grid-cols-3 sm:gap-4">
                                    <dt class="font-medium text-gray-900">Go Version</dt>
                                    <dd class="text-gray-700 sm:col-span-2 overflow-auto"><code>{{ $buildInformation['prometheus']['goVersion'] ?? 'unknown' }}</code></dd>
                                </div>
                                <div class="grid grid-cols-1 gap-1 py-3 sm:grid-cols-3 sm:gap-4">
                                    <dt class="font-medium text-gray-900">Revision</dt>
                                    <dd class="text-gray-700 sm:col-span-2 overflow-auto"><code>{{ $buildInformation['prometheus']['revision'] ?? 'unknown' }}</code></dd>
                                </div>
                            </dl>
                        </div>
                    </div>
                    ...
                    <div class="pt-2 pb-4 px-5">
                        <div class="flow-root">
                            <dl class="-my-3 divide-y divide-gray-100 text-sm">
                                <div class="grid grid-cols-1 gap-1 py-3 sm:grid-cols-3 sm:gap-4">
                                    <dt class="font-medium text-gray-900">Version</dt>
                                    <dd class="text-gray-700 sm:col-span-2 overflow-auto"><code>{{ $buildInformation['alertmanager']['version'] ?? 'unknown' }}</code></dd>
                                </div>
                                <div class="grid grid-cols-1 gap-1 py-3 sm:grid-cols-3 sm:gap-4">
                                    <dt class="font-medium text-gray-900">Go Version</dt>
                                    <dd class="text-gray-700 sm:col-span-2 overflow-auto"><code>{{ $buildInformation['alertmanager']['goVersion'] ?? 'unknown' }}</code></dd>
                                </div>
                                <div class="grid grid-cols-1 gap-1 py-3 sm:grid-cols-3 sm:gap-4">
                                    <dt class="font-medium text-gray-900">Revision</dt>
                                    <dd class="text-gray-700 sm:col-span-2 overflow-auto"><code>{{ $buildInformation['alertmanager']['revision'] ?? 'unknown' }}</code></dd>
                                </div>
                            </dl>
                        </div>
                    </div>
                    ...
                    <div class="pt-2 pb-4 px-5">
                        <div class="flow-root">
                            <dl class="-my-3 divide-y divide-gray-100 text-sm">
                                <div class="grid grid-cols-1 gap-1 py-3 sm:grid-cols-3 sm:gap-4">
                                    <dt class="font-medium text-gray-900">Version</dt>
                                    <dd class="text-gray-700 sm:col-span-2 overflow-auto"><code>{{ $buildInformation['snmpexporter']['version'] ?? 'unknown' }}</code></dd>
                                </div>
                                <div class="grid grid-cols-1 gap-1 py-3 sm:grid-cols-3 sm:gap-4">
                                    <dt class="font-medium text-gray-900">Go Version</dt>
                                    <dd class="text-gray-700 sm:col-span-2 overflow-auto"><code>{{ $buildInformation['snmpexporter']['goVersion'] ?? 'unknown' }}</code></dd>
                                </div>
                                <div class="grid grid-cols-1 gap-1 py-3 sm:grid-cols-3 sm:gap-4">
                                    <dt class="font-medium text-gray-900">Revision</dt>
                                    <dd class="text-gray-700 sm:col-span-2 overflow-auto"><code>{{ $buildInformation['snmpexporter']['revision'] ?? 'unknown' }}</code></dd>
                                </div>
                            </dl>
                        </div>
                    </div>
    ...
    <x-dialog-modal wire:key="prometheus-build-information-modal" wire:model.live="showingPrometheusBuildInformationModal">
        <x-slot name="title">
            {{ __('Prometheus') }}
        </x-slot>
        <x-slot name="content">
            ...
            <div role="alert" class="rounded border-s-4 border-orange-500 bg-orange-50 p-4 mt-4">
                ...
                <p class="mt-2 text-sm text-red-700">
                    This is a core dependency that run by Retia Engine. Custom configuration, upgrade, or modification may affect the whole system performance and accessibility.
                </p>
            </div>
        </x-slot>
        <x-slot name="footer">
            <x-secondary-button wire:click="$toggle('showingPrometheusBuildInformationModal')" wire:loading.attr="disabled" class="mr-4">
                {{ __('Close') }}
            </x-secondary-button>
    ...
    <x-dialog-modal wire:key="alert-manager-build-information-modal" wire:model.live="showingAlertManagerBuildInformationModal">
        <x-slot name="title">
            {{ __('Alert Manager') }}
        </x-slot>
        <x-slot name="content">
        ...
        <x-slot name="footer">
            <x-secondary-button wire:click="$toggle('showingAlertManagerBuildInformationModal')" wire:loading.attr="disabled" class="mr-4">
                {{ __('Close') }}
            </x-secondary-button>
            <a href="https://prometheus.io/docs/alerting/latest/alertmanager" target="_blank" rel="noopener noreferrer">
                <x-button wire:loading.attr="disabled">
                    {{ __('Read More') }}
                </x-button>
            </a>
    ...
    <x-dialog-modal wire:key="snmp-exporter-build-information-modal" wire:model.live="showingSnmpExporterBuildInformationModal">
        <x-slot name="title">
            {{ __('SNMP Exporter') }}
        </x-slot>
        <x-slot name="content">
        ...
        <x-slot name="footer">
            <x-secondary-button wire:click="$toggle('showingSnmpExporterBuildInformationModal')" wire:loading.attr="disabled" class="mr-4">
                {{ __('Close') }}
            </x-secondary-button>
